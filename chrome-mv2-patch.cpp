...
// ============================================================================
// Helper: Check if Google Chrome process is currently running
// ============================================================================
...
// ============================================================================
// Helper: Force-terminate every running Google Chrome process.
// Equivalent to:  taskkill /F /IM chrome.exe /T
// Chrome leaves crashpad/background processes behind that hold the profile
// lock and make later launches appear to "do nothing"; killing all instances
// clears that state before patching or restoring chrome.dll.
// ============================================================================
static void KillAllChromeProcesses() {
    // Chrome respawns crashpad/background processes while it is being torn
    // down, so keep terminating until a full sweep finds nothing (bounded).
    for (int sweep = 0; sweep < 12; ++sweep) {
        ...
        if (pids.empty()) {
            Sleep(3000);  // let terminated PIDs clear from the process list
            return;
        }
        ...
        for (DWORD pid : pids) {
            ...
            if (TerminateProcess(hProc, 1)) {
                std::cout << "    [+] Killed chrome.exe (PID " << pid << ")." << std::endl;
                killed++;
            }
            CloseHandle(hProc);
        }
        if (killed == 0) break;
        Sleep(1500);  // allow respawns and handle cleanup to settle
    }
}
...
static std::wstring LocateChromeDll() {
    wchar_t path[MAX_PATH];

    // Given "...\Google\Chrome\Application", return the chrome.dll under the
    // highest-numbered version subdirectory (or a chrome.dll sitting directly
    // in Application), or "" if none is found. Shared by every install root.
    auto findUnderApplication = [](const std::wstring& appDir) -> std::wstring {
    ...
    // Check Program Files (x64), then Local AppData.
    for (int folder : { CSIDL_PROGRAM_FILES, CSIDL_LOCAL_APPDATA }) {
        if (SUCCEEDED(SHGetFolderPathW(NULL, folder, NULL, 0, path))) {
            std::wstring appDir = std::wstring(path) + L"\\Google\\Chrome\\Application";
            std::wstring found = findUnderApplication(appDir);
            if (!found.empty()) return found;
        }
    }

    // Check current directory
    if (GetFileAttributesW(L"chrome.dll") != INVALID_FILE_ATTRIBUTES) {
        return L"chrome.dll";
    }
...
static int PatchStage151(std::vector<uint8_t>& fileBuffer,
    ...
    std::cout << "[*] Stage 151: Chrome 151 MV2 layout detected (" << located.size()
    ...
    for (const auto& L : located) {
        ...
        if (cur != 0x7F) {
            std::cout << "    [!] Stage 151: " << L.site->name << " unexpected byte 0x"
...
static void ReportLayoutCandidates(const std::vector<uint8_t>& buf,
    ...
    std::cout << "[*] Scanning .text for the IsExtensionAffected skeleton "
    ...
    std::cout << "[*] Skeleton scan found " << total << " candidate site(s)"
...
static bool PatchChromeDllBuffer(std::vector<uint8_t>& fileBuffer) {
    ...
    if (ntHeaders == nullptr) {
        std::cout << "[-] Error: not a valid 64-bit PE image (bad DOS/NT headers or truncated file)." << std::endl;
        return false;
    }
    ...
    if (textSec.rawSize == 0) {
        std::cout << "[-] Error: Could not locate .text section." << std::endl;
        return false;
    }
    ...
    std::cout << "[*] Starting MV2 Patching (ManifestV2Handler architecture)..." << std::endl;
    ...
    // ========================================================================
    std::cout << "[*] Stage 151: probing for the Chrome 151 ManifestV2Handler layout..." << std::endl;
    ...
    if (stage151 == 0) {
        ...
        std::cout << "[!] No known Stage 151 signature matched this chrome.dll." << std::endl;
    ...
    std::cout << "[*] Stage 151 " << ((stage151 == 1) ? "applied."
    ...
    // ========================================================================
    std::cout << "[*] Updating PE Header checksum & clearing Security Directory..." << std::endl;

    PIMAGE_DATA_DIRECTORY secDir = &ntHeaders->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_SECURITY];
    if (secDir->VirtualAddress != 0 || secDir->Size != 0) {
        std::cout << "[+] Clearing Security Directory (RVA: 0x" << std::hex << secDir->VirtualAddress
    ...
    std::cout << "[+] Recalculated PE CheckSum: 0x" << std::hex << newChecksum << std::dec << std::endl;
...
static bool VerifyPatchedFile(const std::wstring& path) {
    ...
    if (!file.is_open()) {
        std::cout << "[-] Verification: could not re-open patched file." << std::endl;
        return false;
    }
    std::streamsize fileSize = file.tellg();
    if (fileSize <= 0) {
        std::cout << "[-] Verification: patched file is empty or unreadable." << std::endl;
        return false;
    }
    file.seekg(0, std::ios::beg);
    std::vector<uint8_t> buf(fileSize);
    if (!file.read((char*)buf.data(), fileSize)) {
        std::cout << "[-] Verification: could not re-read patched file." << std::endl;
        return false;
    }
    file.close();
    ...
    if (ntHeaders == nullptr) {
        std::cout << "[-] Verification: patched file is not a valid PE image on disk." << std::endl;
        return false;
    }
    PIMAGE_SECTION_HEADER secHeader = IMAGE_FIRST_SECTION(ntHeaders);
    ...
    if (textRaw == 0) {
        std::cout << "[-] Verification: .text section not found on disk." << std::endl;
        return false;
    }
    ...
    PIMAGE_DATA_DIRECTORY secDir = &ntHeaders->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_SECURITY];
    if (secDir->VirtualAddress != 0 || secDir->Size != 0) {
        std::cout << "[-] Verification: Security Directory still present on disk (0x"
                  << std::hex << secDir->VirtualAddress << " / 0x" << secDir->Size << std::dec << ")." << std::endl;
        ok = false;
    } else {
        std::cout << "[+] Verification: Security Directory cleared." << std::endl;
    }
    ...
    if (ntHeaders->OptionalHeader.CheckSum != computed) {
        std::cout << "[-] Verification: checksum mismatch (on disk 0x" << std::hex
                  << ntHeaders->OptionalHeader.CheckSum << " vs computed 0x" << computed << std::dec << ")." << std::endl;
        ok = false;
    } else {
        std::cout << "[+] Verification: PE checksum matches (0x" << std::hex << computed << std::dec << ")." << std::endl;
    }
    ...
    for (const auto& wp : g_writtenPatches) {
        ...
        if ((size_t)raw + wp.second.size() > buf.size() ||
            ...
            std::cout << "[-] Verification: site RVA 0x" << std::hex << wp.first << std::dec
                      << " does NOT match the expected patch on disk!" << std::endl;
            ok = false;
        } else {
            std::cout << "[+] Verification: patch site RVA 0x" << std::hex << wp.first << std::dec
...
static void PrintUsage() {
    std::cout << "Usage: chrome-mv2-patch.exe [path\\to\\chrome.dll] [options]\n"
                 ...
                 "With no path, the newest installed chrome.dll is located automatically.\n"
                 "\n"
                 "Options:\n"
...
int wmain(int argc, wchar_t* argv[]) {
    std::cout << "==========================================================" << std::endl;
    std::cout << "        Google Chrome chrome.dll Manifest V2 Patcher       " << std::endl;
    std::cout << "   (Chrome 151 ManifestV2Handler seven-flip edition)       " << std::endl;
    std::cout << "                    v" APP_VERSION_STR "                       " << std::endl;
    std::cout << "==========================================================" << std::endl;
    ...
    if (!IsElevatedAdmin()) {
        return Fail("[!] WARNING: Administrator privileges are REQUIRED to modify chrome.dll in Program Files!\n"
                    "    Please right-click chrome-mv2-patch.exe and select 'Run as administrator'.");
    }

    if (targetPath.empty()) {
        std::cout << "[*] Searching for installed Google Chrome chrome.dll..." << std::endl;
        targetPath = LocateChromeDll();
    }

    if (targetPath.empty()) {
        return Fail("[-] Error: Could not locate chrome.dll automatically.\n"
                    "    Usage: chrome-mv2-patch.exe [path\\to\\chrome.dll] [--restore]");
    }

    std::wcout << L"[+] Target file: " << targetPath << std::endl;
    std::wstring ver = GetFileVersion(targetPath);
    if (!ver.empty()) {
        std::wcout << L"[+] Chrome version detected: " << ver << std::endl;
    }

    std::wstring backupPath = targetPath + L".bak";

    if (restoreMode) {
        std::cout << "[*] Restore mode requested..." << std::endl;
        if (GetFileAttributesW(backupPath.c_str()) == INVALID_FILE_ATTRIBUTES) {
            return Fail("[-] Error: Backup file chrome.dll.bak does not exist.");
        }

        if (IsChromeRunning()) {
            std::cout << "[!] Google Chrome is currently running - terminating all instances..." << std::endl;
            KillAllChromeProcesses();
            if (IsChromeRunning()) {
                return Fail("[-] Chrome processes still active. Aborting restore.");
            }
        }

        if (CopyFileW(backupPath.c_str(), targetPath.c_str(), FALSE)) {
            std::cout << "[SUCCESS] Original chrome.dll successfully restored from backup!" << std::endl;
            return Fail("", 0);  // success: print nothing extra, just pause + exit 0
        } else {
            return Fail("[-] Error restoring file. Code: " + std::to_string(GetLastError()));
        }
    }

    // Terminate all Chrome instances (background/crashpad processes hold the
    // profile lock and the file handle - taskkill /F /IM chrome.exe /T)
    if (IsChromeRunning()) {
        std::cout << "[!] Google Chrome is currently running - terminating all instances..." << std::endl;
        KillAllChromeProcesses();
        if (IsChromeRunning()) {
            std::cout << "[!] Chrome processes still active - retrying..." << std::endl;
            KillAllChromeProcesses();
        }
        if (IsChromeRunning()) {
            return Fail("[-] Chrome processes are still running. Aborting patch.");
        }
    }
    ...
    if (!file.is_open()) {
        return Fail("[-] Error: Failed to open file for reading.");
    }

    std::streamsize fileSize = file.tellg();
    if (fileSize <= 0) {
        return Fail("[-] Error: chrome.dll is empty or its size could not be determined.");
    }
    file.seekg(0, std::ios::beg);

    std::vector<uint8_t> buffer(fileSize);
    if (!file.read((char*)buffer.data(), fileSize)) {
        return Fail("[-] Error reading file contents into memory.");
    }
    file.close();

    std::cout << "[+] Loaded " << fileSize << " bytes from chrome.dll." << std::endl;

    bool isTargetStock = IsStockUnpatchedBuffer(buffer.data(), buffer.size());

    // Smart Backup Management
    if (GetFileAttributesW(backupPath.c_str()) == INVALID_FILE_ATTRIBUTES) {
        std::cout << "[*] Creating initial backup copy: chrome.dll.bak ..." << std::endl;
        if (!CopyFileW(targetPath.c_str(), backupPath.c_str(), TRUE)) {
            return Fail("[-] Error creating backup file. Code: " + std::to_string(GetLastError()));
        }
        std::cout << "[+] Initial backup created successfully." << std::endl;
    } else {
        if (isTargetStock) {
            std::cout << "[*] New Chrome version / unpatched stock detected! Updating chrome.dll.bak ..." << std::endl;
            CopyFileW(targetPath.c_str(), backupPath.c_str(), FALSE);
            std::cout << "[+] Backup updated to latest stock version." << std::endl;
        } else {
            std::cout << "[*] Previously patched chrome.dll detected. Restoring clean stock from backup before re-patching..." << std::endl;
            if (CopyFileW(backupPath.c_str(), targetPath.c_str(), FALSE)) {
                ...
                if (rFile.is_open()) {
                    ...
                    rFile.close();
                    std::cout << "[+] Restored clean stock chrome.dll into memory buffer." << std::endl;
                }
            } else {
                std::cout << "[-] Backup restore failed (chrome.dll.bak missing? code "
    ...
    if (!PatchChromeDllBuffer(buffer)) {
        return Fail("[WARNING] No patches were applied.", 0);
    }

    // Write Modified Buffer Back to Disk
    std::ofstream outFile(targetPath, std::ios::binary | std::ios::trunc);
    if (!outFile.is_open()) {
        return Fail("[-] Error: Failed to open file for writing (permission denied or file locked).\n"
    ...
    outFile.close();
    if (!outFile) {
        return Fail("[-] Error: Failed to write patched bytes to chrome.dll (disk full or file locked).");
    }

    std::cout << "[*] Verifying patched chrome.dll on disk..." << std::endl;
    if (!VerifyPatchedFile(targetPath)) {
        return Fail("[-] On-disk verification FAILED! Use 'chrome-mv2-patch.exe --restore' to revert.");
    }

    std::cout << "==========================================================" << std::endl;
    std::cout << "[SUCCESS] chrome.dll successfully patched and verified on disk!" << std::endl;
