...
- **Linux** — shipping. Targets **Chrome 151** (64-bit ELF, build 151.0.7922.108). Five gate sites derived and verified: the shared predicate `MV2DeprecationImpactChecker::IsExtensionAffected`, `ShouldBlockExtensionInstallation`/`ShouldBlockExtensionEnable`, and the two inlined copies in `StandardManagementPolicyProvider::{UserMayInstall,MustRemainDisabled}`. The out-of-line startup paths (`OnExtensionSystemReady`, `MaybeReEnableExtension`) call the shared predicate, so flipping it covers them. The flip was verified by executing the patched predicate in-process: stock returns *affected* for MV2 / *not affected* for MV3; patched returns *not affected* for both, MV3 unchanged.
...
**Table disambiguation, both directions.** The one shared signature means each table partially matches the other's build, and "most sites located wins" resolves it with margin: on the 152 beta ELF `152-linux` satisfies 5/5 and `151-linux` 1/5; on the 151.0.7922.108 stable ELF `151-linux` satisfies 5/5 and `152-linux` 1/5. Both were confirmed by running the shipping engine against a scratch copy, not just `--verify`. This is the guard-rail the third-party beta signatures in §7 lacked: a beta-derived entry is safe to ship precisely because it is container- and milestone-tagged and every site is accepted only on an exact match count, so on a stable build it declines instead of matching an unrelated function.

**Not yet runtime-tested.** The 151 entry was confirmed by executing the patched predicate in-process and a headless startup (§4c); this one is verified statically (on-disk bytes + engine acceptance) on a Windows host. Chrome 152 beta on a Linux box is the remaining check before treating it as proven.
